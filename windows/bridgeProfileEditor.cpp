...
void BridgeProfileEditor::handleDrawListBox(PDRAWITEMSTRUCT item) {
	...
	if (handleGetDriverInfo((unsigned int)item->itemData, &info)) {
		...
		if (item->rcItem.bottom - item->rcItem.top < 66) {
			...
			oldBmp = SelectObject(m_hdcTemp, m_bridgeLogos[item->itemData + 3]);
			sizeUsed = 16;
		}
		else {
			oldBmp = SelectObject(m_hdcTemp, m_bridgeLogos[item->itemData]);
		}
		SetStretchBltMode(item->hDC, HALFTONE);
		SetBrushOrgEx(item->hDC, 0, 0, NULL);
		TransparentBlt(item->hDC, item->rcItem.left + 1, item->rcItem.top + 1, bitmapSize - 2, bitmapSize - 2, m_hdcTemp, 0, 0, sizeUsed, sizeUsed, 0x00FF00);
